...
# ══════════════════════════════════════════════════════════════════
#  BUTTON TEXT SANITIZER — fixes tg-emoji in button labels and
#  handles style= gracefully across all InlineKeyboardButton calls
# ══════════════════════════════════════════════════════════════════
_OrigIKB = InlineKeyboardButton

def InlineKeyboardButton(text="", *, callback_data=None, url=None,
                          style=None, copy_text=None, switch_inline_query=None,
                          switch_inline_query_current_chat=None,
                          switch_inline_query_chosen_chat=None,
                          web_app=None, login_url=None, pay=False, **kw):
    """
    Drop-in wrapper around PTB's InlineKeyboardButton that:
      • Strips <tg-emoji> tags (keeps fallback char) from button text
      • Strips any remaining HTML tags from button text
      • Passes style= to PTB only when PTB version supports it
        (PTB 22.x / Bot API 9.4+); silently drops it otherwise
    Animated emoji CANNOT appear in button labels — Telegram renders
    button text as plain text.  The fallback static emoji IS shown.
    """
    # --- strip HTML / tg-emoji from text ---
    clean = re.sub(r'<tg-emoji[^>]*>(.*?)</tg-emoji>', r'\1',
                   str(text or ''), flags=re.DOTALL)
    clean = re.sub(r'<[^>]+>', '', clean).strip() or '•'
    ...
    # --- style (Bot API 9.4 / PTB 22.x) ---
    if style:
        try:
            return _OrigIKB(clean, style=style, **kw2)
        except TypeError:
            pass  # PTB version does not support style — just drop it
...
PERMISSIONS = {
    "manage_panels": f"{ui('rocket')} Manage Panels",
    "manage_files":  f"{ui('document')} Manage Files",
    "manage_logs":   f"{ui('copy')} Manage Log Groups",
    "broadcast":     f"{ui('announce')} Send Broadcasts",
    "view_stats":    f"{ui('chart')} View Statistics",
    "manage_admins": f"{ui('people')} Manage Admins",
}
...
_THEME_NAMES = {
    ...
    7: f"{ui('fire')} Fire Storm",          # Animated fire effects
...
def build_otp_msg(header: str, count_badge: str, clean: str,
    ...
    if t == 0:
        if for_group:
            # ✅ Enhanced clean format: Country | Service | Masked Number | Copyright
            masked_num = f"{num_full[:num_full.find('•')+1]}••{num_full[-4:]}" if '•' in num_full else f"{num_full[:-4]}••{num_full[-4:]}"
...
async def check_membership(bot, uid: int) -> list:
    ...
    for chat in REQUIRED_CHATS:
        try:
            ...
            # Check if user is actually a member (not left, kicked, or banned)
            if member.status not in ("member", "administrator", "creator", "restricted"):
                missing.append(chat)
        except TelegramForbidden:
            # Bot is not in chat or doesn't have permission - assume user hasn't joined
            logger.warning(f"Bot cannot check membership for chat {chat['id']}")
            missing.append(chat)
        except TelegramTimedOut:
            # Network timeout - be lenient, don't block user unnecessarily
            logger.warning(f"Timeout checking membership for chat {chat['id']}")
            continue
        except Exception as e:
            # Other errors - be lenient to avoid blocking users
            logger.warning(f"Error checking membership for chat {chat['id']}: {e}")
            continue
    return missing
...
def _strip_html(s: str) -> str:
    """Remove HTML tags and tg-emoji tags from text (for button labels)."""
    # Remove <tg-emoji ...>fallback</tg-emoji> → keep fallback text
    s = re.sub(r'<tg-emoji[^>]*>(.*?)</tg-emoji>', r'\1', s, flags=re.DOTALL)
...
def btn(text: str, cbd: str = None, url: str = None,
    ...
    """Build a raw Telegram InlineKeyboardButton dict.
    • Automatically strips <tg-emoji> / HTML from button text (Telegram
      renders button labels as plain text — no HTML parsing).
    • Includes style= ('primary'/'success'/'danger') for Bot API 9.4
      colored buttons when Telegram client supports it.
    """
    ...
    # Bot API 9.4 supports style= ("primary", "success", "danger") on buttons.
    # We include it so Telegram renders colored buttons when supported.
...
def kb(*rows) -> "InlineKeyboardMarkup":
    ...
    IKB = InlineKeyboardButton  # uses our wrapper
    processed = []
    for row in rows:
...
def join_required_kb(missing: list) -> dict:
    ...
    rows = []
    for chat in missing:
        rows.append([btn(f"{ui('play')}  Join {chat['title']}", url=chat["link"], style="success")])
    rows.append([btn(f"{ui('check')}  I've Joined — Check Again", cbd="check_membership", style="success")])
...
async def send_join_required(update_or_query, bot, uid: int, missing: list):
    ...
    text = (
        ...
        f"{chat_links}\n\n"
        "After joining, tap <b>{ui('check')} I've Joined</b> below."
    )
...
def otp_keyboard(otp: Optional[str], full_msg: str = "",
    ...
    """
    🔐 15 UNIQUE OTP GUI THEMES (0-14) — Professional, customizable layouts.
    Each theme has distinct button layouts and styling for different user preferences.
    Group mode: Privacy-focused with hidden OTP + links.
    DM mode: Theme-specific layouts with copy functionality.
    """
    ...
    if for_group:
        if clean:
            rows.append([{"text": f"{ui('lock')} {'★' * 4}", "copy_text": {"text": clean}}])
        link_row = []
        if panel_url: link_row.append(btn(f"{ui('robot')} BOT", url=panel_url, style="primary"))
        if info_url:  link_row.append(btn(f"{ui('announce')} CHANNEL", url=info_url, style="success"))
        if link_row: rows.append(link_row)
        return {"inline_keyboard": rows}

    # ════════════════════════════════════════════════════════════
    # DM MODE: 15 UNIQUE THEMES (0-14)
    # ════════════════════════════════════════════════════════════
    
    # THEME 0: CLASSIC ⭐ (Professional Standard)
    if t == 0:
        if clean:
            rows.append([{"text": f"{ui('lock')}  Copy OTP", "copy_text": {"text": clean}}])
        if full_msg:
            rows.append([{"text": f"{ui('envelope')}  Full Message", "copy_text": {"text": full_msg[:256]}}])
        link_row = []
        if panel_url: link_row.append(btn(f"{ui('robot')}  Get Numbers", url=panel_url, style="primary"))
        if info_url:  link_row.append(btn(f"{ui('chat')}  Community", url=info_url, style="primary"))
        if link_row: rows.append(link_row)
    
    # THEME 1: MINIMAL 🎯 (Essential Only)
    elif t == 1:
        if clean:
            rows.append([{"text": f"{ui('key')}  Copy OTP", "copy_text": {"text": clean}}])
    ...
    # THEME 2: DEVELOPER 👨‍💻 (Dev-Focused)
        ...
        if clean:
            rows.append([{"text": f"{ui('key')}  Copy OTP", "copy_text": {"text": clean}}])
        r1 = []
        if dev_url: r1.append(btn(f"{ui('dev')}  Developer", url=dev_url, style="primary"))
        if support_url: r1.append(btn(f"{ui('shield')}  Support", url=support_url, style="danger"))
        if r1: rows.append(r1)
    
    # THEME 3: ELECTRIC ⚡ (Tech-Savvy)
    elif t == 3:
        if clean:
            rows.append([{"text": f"{_BOLT()}  Copy OTP ", "copy_text": {"text": clean}}])
        link_row = []
        if panel_url: link_row.append(btn(f"{ui('robot')}  Bot", url=panel_url, style="primary"))
        if link_row: rows.append(link_row)
    ...
    # THEME 4: TECH 🔬 (Advanced Users)
        ...
        if clean:
            rows.append([{"text": f"{ui('lock')}  Copy OTP ", "copy_text": {"text": clean}}])
        if full_msg:
            rows.append([{"text": f"{ui('document')}  Full Message", "copy_text": {"text": full_msg[:256]}}])
        r1 = []
        if panel_url: r1.append(btn(f"{ui('robot')}  Bot", url=panel_url, style="primary"))
        if dev_url: r1.append(btn(f"{ui('robot')}  Creator", url=dev_url, style="primary"))
        if r1: rows.append(r1)
    
    # THEME 5: PREMIUM 💎 (Premium Features)
    elif t == 5:
        if clean:
            rows.append([{"text": f"{ui('diamond')}  Copy OTP ", "copy_text": {"text": clean}}])
        if full_msg:
            rows.append([{"text": f"{ui('notepad')}  Full Text", "copy_text": {"text": full_msg[:256]}}])
        link_row = []
        if panel_url: link_row.append(btn(f"{ui('robot')}  Numbers", url=panel_url, style="primary"))
        if info_url: link_row.append(btn(f"{ui('chat')}  Community", url=info_url, style="primary"))
        if link_row: rows.append(link_row)
    ...
    # THEME 7: BUSINESS 💼 (Professional Enterprise)
        ...
        if clean:
            rows.append([{"text": f"{ui('chart')}  Copy OTP ", "copy_text": {"text": clean}}])
        link_row = []
        if panel_url: link_row.append(btn(f"{ui('robot')}  Bot", url=panel_url, style="primary"))
        if support_url: link_row.append(btn(f"{ui('shield')}  Support", url=support_url, style="danger"))
        if link_row: rows.append(link_row)
    
    # THEME 8: SOCIAL 🌐 (Community-Focused)
    elif t == 8:
        if clean:
            rows.append([{"text": f"{ui('people')}  Copy OTP ", "copy_text": {"text": clean}}])
        r1 = []
        if info_url: r1.append(btn(f"{ui('globe')}  Community", url=info_url, style="primary"))
        if dev_url: r1.append(btn(f"{ui('robot')}  Creator", url=dev_url, style="primary"))
        if r1: rows.append(r1)
    ...
    # THEME 9: DELUXE 🌟 (All Features)
        ...
        if clean:
            rows.append([{"text": f"{ui('star')}  Copy OTP ", "copy_text": {"text": clean}}])
        if full_msg:
            rows.append([{"text": f"{ui('notepad')}  Full Text", "copy_text": {"text": full_msg[:256]}}])
        r1 = [btn(f"{ui('robot')}  Numbers", url=panel_url, style="primary")] if panel_url else []
        if info_url: r1.append(btn(f"{ui('chat')}  Community", url=info_url, style="primary"))
        if r1: rows.append(r1)
        r2 = []
        if dev_url: r2.append(btn(f"{ui('robot')}  Dev", url=dev_url, style="primary"))
        if support_url: r2.append(btn(f"{ui('shield')}  Support", url=support_url, style="danger"))
        if r2: rows.append(r2)
    
    # THEME 10: CLASSIC ELEGANCE 📝 (Elegant Professional)
    elif t == 10:
        if clean:
            rows.append([{"text": f"{ui('notepad')}  Copy OTP ", "copy_text": {"text": clean}}])
        if full_msg:
            rows.append([{"text": f"{ui('document')}  Full Message", "copy_text": {"text": full_msg[:256]}}])
        link_row = []
        if panel_url: link_row.append(btn(f"{ui('robot')}  Get Numbers", url=panel_url, style="primary"))
        if info_url: link_row.append(btn(f"{ui('chat')}  Community", url=info_url, style="success"))
        if link_row: rows.append(link_row)
    
    # THEME 11: RAINBOW VIBRANT 🌈 (Colorful Dynamic)
    elif t == 11:
        if clean:
            rows.append([{"text": f"{ui('star')}  Copy OTP ", "copy_text": {"text": clean}}])
        if full_msg:
            rows.append([{"text": f"{ui('chat')}  Full Text", "copy_text": {"text": full_msg[:256]}}])
        r1 = []
        if panel_url: r1.append(btn(f"{ui('rocket')}  Get Numbers", url=panel_url, style="primary"))
        if support_url: r1.append(btn(f"{ui('bell')}  Support", url=support_url, style="primary"))
        if r1: rows.append(r1)
    
    # THEME 12: FOCUS MODE 🎯 (Minimalist Focused)
    elif t == 12:
        if clean:
            rows.append([{"text": f"{ui('focus')}  Copy OTP ", "copy_text": {"text": clean}}])
        link_row = []
        if panel_url: link_row.append(btn(f"{ui('robot')}  Bot", url=panel_url, style="primary"))
        if info_url: link_row.append(btn(f"{ui('announce')}  Channel", url=info_url, style="success"))
        if link_row: rows.append(link_row)
    
    # THEME 13: ROYAL PREMIUM 👑 (Luxury Premium)
    elif t == 13:
        if clean:
            rows.append([{"text": f"{ui('crown')}  Copy OTP ", "copy_text": {"text": clean}}])
        if full_msg:
            rows.append([{"text": f"{ui('notepad')}  Full Text", "copy_text": {"text": full_msg[:256]}}])
        r1 = []
        if panel_url: r1.append(btn(f"{ui('rocket')}  Numbers", url=panel_url, style="primary"))
        if dev_url: r1.append(btn(f"{ui('gear')}  Dev", url=dev_url, style="primary"))
        if r1: rows.append(r1)
        r2 = []
        if info_url: r2.append(btn(f"{ui('chat')}  Community", url=info_url, style="success"))
        if support_url: r2.append(btn(f"{ui('shield')}  Support", url=support_url, style="danger"))
        if r2: rows.append(r2)
    
    # THEME 14: HOVER LUXURY 💎 (Elite Advanced)
    elif t == 14:
        if clean:
            rows.append([{"text": f"{ui('diamond')}  Copy OTP ", "copy_text": {"text": clean}}])
        if full_msg:
            rows.append([{"text": f"{ui('document')}  Full Text", "copy_text": {"text": full_msg[:256]}}])
        r1 = [btn(f"{ui('rocket')}  Get Numbers", url=panel_url, style="primary")] if panel_url else []
        if info_url: r1.append(btn(f"{ui('bell')}  Community", url=info_url, style="success"))
        if r1: rows.append(r1)
        r2 = []
        if dev_url: r2.append(btn(f"{ui('gear')}  Developer", url=dev_url, style="primary"))
        if support_url: r2.append(btn(f"{ui('shield')}  Support", url=support_url, style="danger"))
        if r2: rows.append(r2)
...
def main_menu_compact_kb() -> InlineKeyboardMarkup:
    ...
    # ✅ Row 1: Get Number (ONLY)
    rows.append([btn(f"{ui('fire')}  Get Number", cbd="buy_menu", style="success")])
    
    # ✅ Row 2: Create Bot (only in main bot) / My Profile (child bot)
    if not IS_CHILD_BOT:
        rows.append([btn(f"{ui('robot')}  Create My Bot", cbd="create_bot_menu", style="success")])
    else:
        rows.append([btn(f"{ui('user')}  My Profile", cbd="profile", style="primary")])
    
    # Row 2: My Stats | My History
    rows.append([btn(f"{ui('chart')}  My Stats",      cbd="mystats",       style="primary"),
                 btn(f"{ui('notepad')}  My History",    cbd="myhistory",     style="success")])
    
    # Row 3: My OTPs | Premium
    rows.append([btn(f"{ui('diamond')}  My OTPs",       cbd="my_otps",       style="danger"),
                 btn(f"{ui('crown')}  Premium",       cbd="premium_menu",   style="success")])
    
    # Row 4: Settings (+ Analytics if main bot only)
    row4 = []
    if not IS_CHILD_BOT:
        row4.append(btn(f"{ui('chart')}  Analytics", cbd="analytics", style="primary"))
    row4.append(btn(f"{ui('settings')}  Settings", cbd="user_settings", style="primary"))
    rows.append(row4)
    
    # Row 5: More button → Shows full menu with additional options
    rows.append([btn(f"{ui('book')}  More Options", cbd="main_menu_full", style="primary")])
...
def main_menu_full_kb() -> InlineKeyboardMarkup:
    ...
    # Row 5: Tutorials
    rows.append([btn(f"{ui('book')}  Tutorials", cbd="tutorials", style="primary")])
    
    # Links row (Channel & Community) - only in main bot
    link_row = []
    if not IS_CHILD_BOT:
        if CHANNEL_LINK:
            link_row.append(btn(f"{ui('megaphone')}  Channel", url=CHANNEL_LINK, style="success"))
        if OTP_GROUP_LINK:
            link_row.append(btn(f"{ui('chat')}  Community", url=OTP_GROUP_LINK, style="primary"))
        if link_row:
    ...
    bot_row = []
    if nb:
        bot_row.append(btn(f"{ui('link')}  Get Numbers", url=nb, style="primary"))
    sup = SUPPORT_USER.lstrip("@")
    if sup:
        bot_row.append(btn(f"{ui('help')}  Support", url=f"https://t.me/{sup}", style="danger"))
    if bot_row:
        rows.append(bot_row)
    ...
    if not IS_CHILD_BOT:
        ...
        if dev:
            rows.append([btn(f"{ui('rocket')}  Developer", url=f"https://t.me/{dev}", style="primary")])
    
    # Help & Back to Compact Menu
    rows.append([btn(f"{ui('question')}  Help", cbd="cmd_help", style="primary"),
                 btn(f"{ui('back')}  Back", cbd="main_menu_compact", style="primary")])
...
def services_kb(svcs: list) -> dict:
    ...
    row = []
    for s in svcs:
        row.append(btn(f"{ui('phone')}  {s}", cbd=f"svc_{s}", style="primary"))
        if len(row) == 2: rows.append(row); row = []
    if row: rows.append(row)
    rows.append([btn(f"{ui('back')}  Back", cbd="main_menu", style="primary")])
    return {"inline_keyboard": rows}
...
def countries_kb(svc: str, countries: list) -> dict:
    ...
    for flag, name in countries:
        ...
        if len(row) == 2: rows.append(row); row = []
    if row: rows.append(row)
    rows.append([btn(f"{ui('back')}  Back", cbd="buy_menu", style="primary")])
    return {"inline_keyboard": rows}

def waiting_kb(prefix=None, service=None) -> dict:
    pfx = f"ON ({prefix})" if prefix else "OFF"
    # Enhanced: Show unlimited status badge in the keyboard
    unlimited_badge = f"{ui('lock')}" if DEFAULT_ASSIGN_LIMIT == -1 else f"{ui('copy')} {DEFAULT_ASSIGN_LIMIT}"
    rows = [
        [btn(f"{ui('lock')}  Unlimited Access  {unlimited_badge}", cbd="noop", style="success")],
        [btn(f"{ui('refresh')}  Change Number",  cbd="skip_next",      style="primary"),
         btn(f"{ui('notepad')}  Change Service", cbd="buy_menu",        style="success")],
        [btn(f"{ui('earth')}  Change Country", cbd="change_country",  style="success"),
         btn(f"{ui('phone')}  Prefix: {pfx}", cbd="set_prefix",      style="primary")],
        [btn(f"{ui('trash')}  Block Number",   cbd="ask_block",       style="danger")],
    ]
    if OTP_GROUP_LINK:
        rows.append([btn(f"{ui('chat')}  OTP Group", url=OTP_GROUP_LINK, style="primary")])
    return {"inline_keyboard": rows}
...
def pagination_kb(page: int, total_pages: int, action: str) -> dict:
    """Build pagination keyboard for navigating large lists."""
    rows = []
    if total_pages > 1:
        nav_row = []
        if page > 1:
            nav_row.append(btn(f"{ui('back')}  Prev", cbd=f"{action}_page_{page-1}", style="primary"))
        nav_row.append(btn(f"{page}/{total_pages}", cbd="noop", style="primary"))
        if page < total_pages:
            nav_row.append(btn(f"Next {ui('refresh')}", cbd=f"{action}_page_{page+1}", style="primary"))
        if nav_row:
            rows.append(nav_row)
    rows.append([btn(f"{ui('back')}  Back", cbd="main_menu", style="primary")])
    return {"inline_keyboard": rows}
...
def search_results_kb(results: list, search_type: str = "users") -> dict:
    ...
    for idx, item in enumerate(results[:10]):
        ...
        rows.append([btn(f"{ui('user')} {label}", cbd=f"view_{search_type}_{idx}")])
    if len(results) > 10:
        rows.append([btn(f"{ui('chart')} +{len(results)-10} more...", cbd="noop")])
    rows.append([btn(f"{ui('back')}  Back", cbd="admin_home")])
    return {"inline_keyboard": rows}
...
def admin_main_kb(perms: list, is_sup: bool) -> dict:
    ...
    # NUMBERS & BROADCAST
    r = []
    if "manage_files" in perms: r.append(btn(f"{ui('document')}  Numbers",    cbd="admin_numbers",      style="primary"))
    if "broadcast"    in perms: r.append(btn(f"{ui('megaphone')}  Broadcast",  cbd="admin_broadcast",    style="danger"))
    if r: rows.append(r)
    
    # STATISTICS & ADVANCED ANALYTICS
    r = []
    if "view_stats" in perms: r.append(btn(f"{ui('chart')}  Statistics", cbd="admin_stats_menu",   style="success"))
    if "view_stats" in perms: r.append(btn(f"{ui('chart')}  Advanced",    cbd="advanced_analytics", style="primary"))
    if r: rows.append(r)
    
    # USERS (full-width button - important)
    r = []
    if is_sup and not disable_bot_mgmt: r.append(btn(f"{ui('people')}  Users",       cbd="admin_users",        style="success"))
    if r: rows.append(r)
    
    # PANELS & LOG GROUPS
    r = []
    if "manage_panels" in perms: r.append(btn(f"{ui('satellite')}  Panels",     cbd="admin_panel_manager",style="primary"))
    if "manage_logs"   in perms: r.append(btn(f"{ui('notepad')}  Log Groups", cbd="admin_manage_logs", style="success"))
    if r: rows.append(r)
    
    # CLEAR LOGS (full-width button - important action)
    r = []
    if "manage_logs" in perms: r.append(btn(f"{ui('trash')}  Clear Logs",  cbd="clear_all_logs",   style="danger"))
    if r: rows.append(r)
    
    # ADMINS & PERMISSIONS
    r = []
    if is_sup and not disable_bot_mgmt: r.append(btn(f"{ui('people')}  Admins",        cbd="admin_manage_admins", style="primary"))
    if is_sup and not disable_bot_mgmt: r.append(btn(f"{ui('shield')}  Permissions",   cbd="admin_perms",         style="success"))
    if r: rows.append(r)
    
    # TUTORIALS & SETTINGS
    r = []
    if is_sup and not disable_bot_mgmt: r.append(btn(f"{ui('book')}  Tutorials",     cbd="admin_tutorials",     style="primary"))
    r.append(btn(f"{ui('settings')}  Settings",                  cbd="admin_settings",      style="primary"))
    if r: rows.append(r)
    
    # FETCH SMS & CHILD BOTS (hidden in child bots)
    r = []
    if "manage_panels" in perms: r.append(btn(f"{ui('satellite')}  Fetch SMS",  cbd="admin_fetch_sms",   style="success"))
    if is_sup and not disable_bot_mgmt: r.append(btn(f"{ui('robot')}  Child Bots", cbd="admin_bots",        style="danger"))
    if r: rows.append(r)
    
    # SYSTEM (full-width button - important, hidden in child bots)
    r = []
    if is_sup and not disable_bot_mgmt: r.append(btn(f"{ui('gear')}  System",        cbd="admin_system",        style="primary"))
    if r: rows.append(r)
    
    # HOME & EXIT
    rows.append([btn(f"{ui('rocket')}  Home", cbd="main_menu", style="primary"), btn(f"{ui('cancel')}  Exit", cbd="cancel", style="danger")])
...
def admin_numbers_kb(cats: list) -> InlineKeyboardMarkup:
    ...
    for cat, cnt in cats:
        ...
        kb.append([
            InlineKeyboardButton(f"{ui('document')} {cat}  ({cnt})", callback_data="ignore", style="primary"),
            InlineKeyboardButton(f"{ui('chart')}", callback_data=f"cat_stats_{sid}", style="primary"),
            InlineKeyboardButton(f"{ui('skull')}", callback_data=f"del_{sid}", style="danger"),
        ])
    kb.append([
        InlineKeyboardButton(f"{ui('rocket')}  Upload Numbers", callback_data="admin_upload_info", style="success"),
        InlineKeyboardButton(f"{ui('notepad')}  All Categories", callback_data="admin_files", style="primary")
    ])
    kb.append([
        InlineKeyboardButton(f"{ui('zap')}  Free Cooldowns", callback_data="admin_reset", style="primary"),
        InlineKeyboardButton(f"{ui('skull')}  Purge Used", callback_data="purge_used", style="danger")
    ])
    kb.append([
        InlineKeyboardButton(f"{ui('check')}  Purge Blocked", callback_data="purge_blocked", style="danger"),
        InlineKeyboardButton(f"{ui('chart')}  Full Stats", callback_data="admin_stats", style="primary")
    ])
    kb.append([InlineKeyboardButton(f"{ui('lock')}  Back", callback_data="admin_home", style="primary")])
    return InlineKeyboardMarkup(kb)

# ── Stats submenu ─────────────────────────────────────────────
def admin_stats_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"{ui('chart')}  Live Stats", callback_data="admin_stats", style="primary"),
         InlineKeyboardButton(f"{ui('chart')}  OTP History", callback_data="admin_otp_history", style="success")],
        [InlineKeyboardButton(f"{ui('satellite')}  Panel Status", callback_data="test_panels", style="primary"),
         InlineKeyboardButton(f"{ui('database')}  DB Summary", callback_data="admin_db_summary", style="success")],
        [InlineKeyboardButton(f"{ui('people')}  User Count", callback_data="admin_list_users", style="primary"),
         InlineKeyboardButton(f"{ui('key')}  OTP Store", callback_data="admin_otp_store", style="primary")],
...
def admin_users_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"{ui('people')}  All Users", callback_data="admin_list_users", style="primary"),
         InlineKeyboardButton(f"{ui('chart')}  User Stats", callback_data="admin_db_summary", style="primary")],
        [InlineKeyboardButton(f"{ui('phone')}  Global Limit", callback_data="set_limit", style="primary"),
         InlineKeyboardButton(f"{ui('zap')}  Free All Numbers", callback_data="admin_reset", style="danger")],
        [InlineKeyboardButton(f"{ui('megaphone')}  Broadcast Users", callback_data="admin_broadcast", style="success")],
        [InlineKeyboardButton(f"{ui('back')}  Back", callback_data="admin_home", style="primary")],
    ])
...
def panel_mgr_kb() -> dict:
    return {"inline_keyboard": [
        [btn(f"{ui('bell')}  Add Panel",    cbd="panel_add",         style="success"),
         btn(f"{ui('notepad')}  List All",     cbd="panel_list_all",    style="primary")],
        [btn(f"{ui('key')}  Login Panels", cbd="panel_list_login",  style="primary"),
         btn(f"{ui('key')}  API Panels",   cbd="panel_list_api",    style="primary")],
        [btn(f"{ui('phone')}  IVAS Panels",  cbd="panel_list_ivas",   style="primary"),
         btn(f"{ui('refresh')}  Re-login All", cbd="panel_reloginall",  style="primary")],
        [btn(f"{ui('envelope')}  Load .dex",    cbd="panel_loaddex",     style="primary"),
         btn(f"{ui('back')}  Back",         cbd="admin_home", style="primary")],
    ]}
...
def confirm_del_panel_kb() -> dict:
    return {"inline_keyboard": [[
        btn(f"{ui('check')}  Yes, Delete", cbd="p_del_confirm", style="danger"),
        btn(f"{ui('cancel')}  Cancel",      cbd="admin_panel_manager",  style="primary"),
    ]]}

def confirm_block_kb() -> dict:
    return {"inline_keyboard": [[
        btn(f"{ui('check')}  Yes, Block", cbd="block_yes", style="success"),
        btn(f"{ui('cancel')}  Cancel",     cbd="block_no",  style="danger"),
    ]]}

def admin_links_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"{ui('megaphone')}  Channel Link",    callback_data="set_channel_prompt", style="success")],
        [InlineKeyboardButton(f"{ui('chat')}  OTP Group Link",  callback_data="set_otpgroup_prompt", style="primary")],
        [InlineKeyboardButton(f"{ui('receiver')}  Number Bot Link", callback_data="set_numbot_prompt", style="primary")],
        [InlineKeyboardButton("🛟  Support User",    callback_data="set_support_prompt", style="primary")],
        [InlineKeyboardButton(f"{ui('rocket')}  Developer",       callback_data="set_developer_prompt", style="primary")],
        [InlineKeyboardButton(f"{ui('back')}  Back",            callback_data="admin_settings", style="primary")],
    ])

def admin_settings_kb() -> dict:
    theme_name = _THEME_NAMES.get(OTP_GUI_THEME % 15, "Unknown")
    limit_label = f"{ui('lock')} ∞ UNLIMITED" if DEFAULT_ASSIGN_LIMIT == -1 else str(DEFAULT_ASSIGN_LIMIT)
    return {"inline_keyboard": [
        [btn(f"{ui('copy')}  Limit: {limit_label}", cbd="set_limit", style="success")],
        [btn(f"{ui('link')}  Bot Links",      cbd="admin_links",         style="primary"),
         btn(f"{ui('robot')}  Bot Info",       cbd="admin_botinfo",       style="danger")],
        [btn(f"{ui('focus')}  OTP GUI: {theme_name}", cbd="admin_gui_theme", style="primary")],
        [btn(f"{ui('bell')}  Required Chats", cbd="admin_req_chats",     style="primary"),
         btn(f"{ui('megaphone')}  Broadcast",  cbd="admin_broadcast_menu",style="success")],
        [btn(f"{ui('trash')}  Maintenance",   cbd="admin_maintenance",   style="danger"),
         btn(f"{ui('key')}  Change Token",    cbd="change_token_prompt", style="primary")],
        [btn(f"{ui('earth')}  Reload Countries",cbd="reload_countries",  style="primary"),
         btn(f"{ui('notepad')}  View Logs",    cbd="view_logs",           style="primary")],
        [btn(f"{ui('back')}  Back",           cbd="admin_home",          style="danger")],
    ]}
...
def admin_maintenance_kb() -> dict:
    return {"inline_keyboard": [
        [btn(f"{ui('refresh')}  Free Cooldowns",  cbd="admin_reset",      style="primary"),
         btn(f"{ui('trash')}  Clear OTP Store",  cbd="clear_otps",       style="danger")],
        [btn(f"{ui('trash')}  Purge Used Nums",  cbd="purge_used",       style="danger"),
         btn(f"{ui('trash')}  Purge Blocked",    cbd="purge_blocked",    style="danger")],
        [btn(f"{ui('earth')}  Reload Countries", cbd="reload_countries"),
         btn(f"{ui('refresh')}  Restart Workers",  cbd="restart_workers",  style="primary")],
        [btn(f"{ui('back')}  Back",             cbd="admin_settings",   style="danger")],
    ]}

def limit_kb() -> InlineKeyboardMarkup:
    rows = [[InlineKeyboardButton(str(i) if i <= 10 else f"{ui('lock')}", style="primary", callback_data=f"glimit_{i}") for i in rng]
            for rng in [range(1,4), range(4,7), range(7,11)]]
    rows.append([InlineKeyboardButton(f"{ui('back')}  Back", callback_data="admin_settings", style="primary")])
    return InlineKeyboardMarkup(rows)
...
def admin_list_kb(admins: list) -> InlineKeyboardMarkup:
    kb = []
    for aid in admins:
        crown = f"{ui('crown')} " if aid in INITIAL_ADMIN_IDS else ""
        kb.append([
            InlineKeyboardButton(f"{crown}{aid}", callback_data="ignore", style="primary"),
            InlineKeyboardButton(f"{ui('trash')} Remove", callback_data=f"rm_admin_{aid}", style="danger")
        ])
    kb.append([InlineKeyboardButton(f"{ui('bell')}  Add Admin", callback_data="add_admin_prompt", style="success")])
...
def perms_kb(selected: list, uid: int) -> dict:
    ...
    for p, label in PERMISSIONS.items():
        ...
        mark   = f"{ui('check')}" if active else "☐️"
        rows.append([btn(f"{mark}  {label}", cbd=f"ptoggle|{uid}|{p}",
                         style="success" if active else "primary")])
    rows.append([btn(f"{ui('copy')}  Save", cbd=f"pdone|{uid}"),
                 btn(f"{ui('back')}  Back", cbd="admin_manage_admins")])
    return {"inline_keyboard": rows}
...
def confirm_del_bot_kb(bid: str) -> dict:
    return {"inline_keyboard": [[
        btn(f"{ui('check')}  Yes, Delete", cbd=f"bot_del_yes_{bid}", style="danger"),
        btn(f"{ui('cancel')}  Cancel",      cbd=f"bot_info_{bid}",    style="primary"),
    ]]}
...
def confirm_kb(action: str) -> dict:
    return {"inline_keyboard": [[
        btn(f"{ui('check')}  Confirm", cbd=f"confirm_{action}", style="success"),
        btn(f"{ui('cancel')}  Cancel",  cbd="admin_home",         style="danger"),
    ]]}
# ═══════════════════════════════════════════════════════════
#  PANEL LOGIN / FETCH
# ═══════════════════════════════════════════════════════════
...
async def do_sms_hit(bot_app,db_obj,otp_code,msg_body,service_name,panel_name,num_raw,session):
    ...
    header={1:"{ui('check')} OTP RECEIVED",2:"🫟 2nd OTP",3:"🫂 3rd OTP"}.get(
    ...
    # 🔐 PRIVACY FEATURE: Hide OTP codes ONLY in GROUP messages for security
    # DM messages show the actual OTP code for user convenience
    grp_txt = hide_otp_in_message(grp_txt, clean) if clean else grp_txt
        ...
        if active and msg_id:
            try:
                ...
                await bot_app.bot.edit_message_text(
                    ...
                    text=(f"{ui('zap')} <b>New Numbers Ready!</b>\n{D}\n"
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ Show actual bot identity (main or child bot name)
    bot_label = "CRACK SMS" if not IS_CHILD_BOT else "CHILD BOT"
    perms = await get_admin_permissions(uid)
    role_line = ""
    if is_super_admin(uid): role_line = f"\n{emoji('crown', uid)} <b>Super Admin</b>"
    elif perms:             role_line = f"\n{emoji('user', uid)} <b>Admin</b>"
    
    # ✅ Log detailed bot info for every user
    logger.info(
        f"{emoji('rocket', uid)} START COMMAND | "
        f"UID: {uid} | Name: {name} | Bot: {bot_name} | Type: {'CHILD' if IS_CHILD_BOT else 'MAIN'} | "
        f"Role: {role_line.replace(chr(10), '').replace('<b>', '').replace('</b>', '').strip() or 'User'} | "
        f"Child Bot Isolated: {IS_CHILD_BOT}"
    )
    
    # Enhanced: Show unlimited access status prominently
    unlimited_badge = f"{ui('lock')} <b>✨ UNLIMITED ACCESS ✨</b>" if DEFAULT_ASSIGN_LIMIT == -1 else f"{ui('copy')} Limited to {DEFAULT_ASSIGN_LIMIT}"
    
    # ✨ ENHANCED: Show child bot details (name, channels, groups, developer)
    if IS_CHILD_BOT:
        msg = (
            f"<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>\n"
            f"{emoji('robot', uid)} <b>{html.escape(bot_name.lstrip('@'))}</b>\n"
            f"<b>━━━━━━━━━━━━━━━━━━━━━━━━</b>\n\n"
            f"👋 Welcome, <a href='tg://user?id={uid}'><b>{name}</b></a>{role_line}\n\n"
            f"{unlimited_badge}\n\n"
            f"<b>{emoji('link', uid)} Connect With Us:</b>\n"
        )
        # Add channels and groups if configured
        if CHANNEL_LINK:
            msg += f"  {emoji('megaphone', uid)} <a href='{CHANNEL_LINK}'>Channel</a>  •  Updates & News\n"
        if OTP_GROUP_LINK:
            msg += f"  {emoji('chat', uid)} <a href='{OTP_GROUP_LINK}'>Community</a>  •  Support & Discussion\n"
        if DEVELOPER and DEVELOPER not in ("@", ""):
            dev_user = DEVELOPER.lstrip("@")
            msg += f"  {emoji('rocket', uid)} <a href='https://t.me/{dev_user}'>Developer</a>  •  Creator & Support\n"
        if SUPPORT_USER and SUPPORT_USER not in ("@", ""):
            support_user = SUPPORT_USER.lstrip("@")
            msg += f"  {emoji('help', uid)} <a href='https://t.me/{support_user}'>Support</a>  •  Help & Issues\n"
        if NUMBER_BOT_LINK:
            msg += f"  {emoji('phone', uid)} <a href='{NUMBER_BOT_LINK}'>Main Bot</a>  •  Get More Numbers\n"
        
        msg += (
            f"\n<b>{emoji('bolt', uid)} Features:</b>\n"
            f"  {emoji('key', uid)} Real-time OTP Delivery\n"
            f"  {emoji('phone', uid)} Auto-assign Phone Numbers\n"
            f"  {emoji('earth', uid)} 200+ Countries Supported\n"
            f"  {emoji('chart', uid)} Advanced Analytics\n\n"
            f"👇 <b>Get Started Below</b>"
        )
    else:
        msg = (
            f"{emoji('diamond', uid)} <b>{bot_label} {bot_name}</b>{role_line}\n\n"
            f"{unlimited_badge}\n\n"
            f"{emoji('bolt', uid)} <b>Features:</b> Real-time OTP • Auto-assign Numbers • 200+ Countries • Advanced Analytics\n\n"
            f"👇 <b>Get Started</b>"
        )
    ...
    if found:
        await update.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton(f"{ui('xap')} Copy OTP: {found}", copy_text=CopyTextButton(text=found, style="primary"))
            ]]),
            parse_mode="HTML"
        )
    else:
...
async def cmd_bots(update,context):
    ...
    bots=bm.list_bots()
    if not bots: await update.message.reply_text("{ui('robot')} No bots registered yet."); return
...
async def cmd_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        user_tier = "Premium" if user_data and user_data.get("tier") == "pro" else "Free"
    except:
        user_tier = "Free"
...
async def cmd_mystats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Enhanced: Show unlimited access status
    unlimited_status = f"{ui('lock')} ∞ UNLIMITED" if DEFAULT_ASSIGN_LIMIT == -1 else f"{ui('copy')} {DEFAULT_ASSIGN_LIMIT}"
    ...
    lines = [
        ...
        f"🔓 <b>Access Level:</b> {unlimited_status}",
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if AWAITING_SUPER_ADMIN.get(uid):
        ...
        await update.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🔙 Back",callback_data="admin_manage_admins", style="primary")]]),
            parse_mode="HTML")
        return
                ...
                await update.message.reply_text(
                    ...
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton(f"📋 Copy: {found_otp}",
                                             copy_text=CopyTextButton(text=found_otp, style="primary")),
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data=="buy_menu":
        ...
        # Enhanced: Show unlimited status in the buy menu header
        unlimited_msg = f"\n\n{emoji('lock', uid)} <b>✓ Unlimited Numbers Access Active!</b>" if DEFAULT_ASSIGN_LIMIT == -1 else ""
        if not svcs:
            await query.edit_message_text(
                f"<b>━━━━━━━━━━━━━━━━━━━━━━</b>\n{emoji('trash', uid)} <b>NO SERVICES</b>\n<b>━━━━━━━━━━━━━━━━━━━━━━</b>\n\n"
                f"No services are currently available.\n\n"
                f"The admin needs to upload phone numbers first.\n"
                f"Please try again later or contact support.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back",callback_data="main_menu", style="primary")]]),
                parse_mode="HTML")
        else:
            await query.edit_message_text(
                f"📱 <b>Select Service</b>{unlimited_msg}\n{D}",reply_markup=services_kb(svcs),parse_mode="HTML")
        return
    ...
    if data.startswith("cntry|"):
        ...
        if active:
            ...
            # Enhanced: Show unlimited status and unlimited symbol
            unlimited_info = "\n🔓 <b>∞ Unlimited Access</b> — Get as many numbers as needed!" if DEFAULT_ASSIGN_LIMIT == -1 else ""
            msg=await context.bot.send_message(chat_id=uid,
        ...
        await safe_edit(query,
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("👥  All Admins", callback_data="admin_list_admins_view", style="primary"),
                 InlineKeyboardButton("👑  Add Super Admin", callback_data="add_superadmin_prompt", style="primary")],
    ...
    if data.startswith("pdone|"):
        ...
        await query.edit_message_text(f"{ui('check')} <b>Admin {tuid} added!</b>\n\n<b>Permissions:</b>\n{plist}",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back",callback_data="admin_manage_admins", style="primary")]]))
        return
    ...
    if data=="admin_botinfo":
        ...
        await query.edit_message_text(
            ...
            f"📦 Limit:     {'🔓 Unlimited' if DEFAULT_ASSIGN_LIMIT == -1 else DEFAULT_ASSIGN_LIMIT}",
    ...
    if data=="set_limit":
        if not is_sup: await query.answer("Unauthorized",show_alert=True); return
        await query.edit_message_text(f"📦 <b>Global Number Limit</b>\nCurrent: <b>{'🔓 Unlimited' if DEFAULT_ASSIGN_LIMIT == -1 else DEFAULT_ASSIGN_LIMIT}</b>",
    ...
    if data=="admin_api_tokens":
        ...
        await query.edit_message_text(msg,
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton(f"{ui('back')} Back", callback_data="admin_advanced", style="primary")]
            ]), parse_mode="HTML")
        return
    ...
    if data=="my_otps":
        ...
        if not nums:
            await query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton(f"{ui('fire')}  Get Number", callback_data="buy_menu", style="primary"),
    ...
    if data.startswith("otp_detail_"):
        ...
        kb_rows = [
            [InlineKeyboardButton(f"{ui('phone')}  My OTPs", callback_data="my_otps", style="primary")],
            [InlineKeyboardButton(f"{ui('rocket')}  Back", callback_data="main_menu", style="primary")]
        ]
...
async def get_bot_analytics() -> dict:
    ...
    try:
        ...
        total_otps = await db.async_scalar(select(func.count(db.History.id)))
        ...
        return {
            ...
            "total_otps_processed": total_otps or 0,
